...
function endsWith(str, substr) {
  const endIndex = str.length - substr.length;
...
function formatTime(minutes, seconds) {
  const formattedMinutes = String(Math.max(0, Math.floor(minutes))).padStart(
    2,
    '0'
  );
  const formattedSeconds = String(Math.max(0, Math.floor(seconds))).padStart(
    2,
    '0'
  );
  ...
  return str.replace(/[a-zA-Z]/g, function (char) {
    ...
    if (char >= 'A' && char <= 'Z') {
      return String.fromCharCode(
        ((charCode - 'A'.charCodeAt(0) + 13) % 26) + 'A'.charCodeAt(0)
      );
    }
    if (char >= 'a' && char <= 'z') {
      return String.fromCharCode(
